...
class PPGN(object):
    ...
    def creat_gcn_embedd(self):
        ...
        for k in range(len(self.layers_plus)-1):
            temp_embedd = [tf.compat.v1.sparse_tensor_dense_matmul(A_fold_hat[f], embeddings) for f in range(self.n_fold)]
        ...
        with tf.name_scope('propagation'):
            if self.args.NCForMF == 'MF':
                ...
                for i, units in enumerate(self.args.mlp_layers):
                    dense_s = tf.compat.v1.layers.dense(a_s, units, tf.nn.relu, kernel_initializer=initializer,
                                          kernel_regularizer = regularizer, name='dense_s_%d' % i)
                    a_s = tf.compat.v1.layers.dropout(dense_s, self.args.dropout_message)

                    dense_t = tf.compat.v1.layers.dense(a_t, units, tf.nn.relu, kernel_initializer=initializer,
                                          kernel_regularizer=regularizer, name='dense_t_%d' % i)
                    a_t = tf.compat.v1.layers.dropout(dense_t, self.args.dropout_message)


                self.logits_dense_s = tf.compat.v1.layers.dense(inputs=a_s,
                                                      units=1,
                                                      kernel_initializer=initializer,
                                                      kernel_regularizer=regularizer,
                                                      name='logits_dense_s')
                self.logits_dense_t = tf.compat.v1.layers.dense(inputs=a_t,
                                                      units=1,
                                                      kernel_initializer=initializer,
                                                      kernel_regularizer=regularizer,
                                                      name='logits_dense_t')
            ...
            self.optimizer = tf.compat.v1.train.AdamOptimizer(self.args.lr).minimize(self.loss)
