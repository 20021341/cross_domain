...
class CrossTrainer(Trainer):
    ...
    def source_predict(self, batch):
        ### Chuyển list user index và list of item index list trong batch này thành tensor và gán vào gpu
        ### Ví dụ 1 list user là [1, 2, 3] thì list of item list là [[1, 2, 3], [2, 3, 4], [3, 4, 5]]
        user_index, item_index = self.unpack_batch_predict(batch)

        ### Lấy ra các vector embedding tương ứng với index trong source user và source item
        ### source user có shape là (num_source_user, embedding_size)
