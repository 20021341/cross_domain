...
class DataLoader(object):
    ...
    def read_train_data(self, train_file):
        with codecs.open(train_file, "r", encoding="utf-8") as infile:
            train_data = [] # List interaction, mỗi interaction là 1 tuple (user_index, item_index)
            user_set = set() # List user unique
            item_set = set() # List item unique
            ma = {} # Dict user với key là user index và value là set item index
            ma_list = {} # Dict user với key là user index và value là list item index
