...
# dataset part
parser.add_argument('--dataset', type=str, default='movie_video', help='cloth_sport, game video')
...
parser.add_argument('--GNN', type=int, default=4, help='GNN layer.')
...
parser.add_argument('--test_sample_number', type=int, default=100, help='Số lượng random item cho test_data')
...
if "CDRIB" in opt["model"]:
    ...
    source_graph = os.path.join(os.path.dirname(__file__), 'dataset/generated_data', filename, 'train.txt')
    ...
    target_train_data = os.path.join(os.path.dirname(__file__), 'dataset/generated_data', filename, 'train.txt')
...
for epoch in range(1, opt['num_epoch'] + 1):
    ...
    print(f'===Epoch {epoch}===')
    print("Source domain: \tMRR: {:.6f}\tNDCG@5: {:.4f}\tNDCG@10: {:.4f}\tHR@1: {:.6f}\tHR@5: {:.4f}\tHR@10: {:.4f}".format(s_mrr, s_ndcg_5, s_ndcg_10, s_hr_1, s_hr_5, s_hr_10))
    print("Target domain: \tMRR: {:.6f}\tNDCG@5: {:.4f}\tNDCG@10: {:.4f}\tHR@1: {:.6f}\tHR@5: {:.4f}\tHR@10: {:.4f}".format(t_mrr, t_ndcg_5, t_ndcg_10, t_hr_1, t_hr_5, t_hr_10))
    print()
    ...
    if s_dev_score > max(s_dev_score_history):
        best_model_path = model_save_dir + '/best_source_score.pt'
        trainer.save(best_model_path)
        print("new best model saved.")

    if t_dev_score > max(t_dev_score_history):
        best_model_path = model_save_dir + '/best_target_score.pt'
        trainer.save(best_model_path)
        print("new best model saved.")


    file_logger.log("{}\t{:.6f}\t{:.4f}\t{:.4f}".format(epoch, train_loss, s_dev_score, max([s_dev_score] + s_dev_score_history)))


    # save
    if epoch == 1 or epoch % opt['save_epoch'] == 0:
